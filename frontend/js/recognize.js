...
async function uploadAndRecognize(event) {
    const file = event.target.files && event.target.files[0];
    // Reset so selecting the same file again still fires onchange
    event.target.value = '';
    if (!file) return;
    if (!file.type.startsWith('image/')) {
        showError('result-alert', '请选择图片文件');
        return;
    }

    const reader = new FileReader();
    reader.onload = async (e) => {
        const imageData = e.target.result; // data:image/...;base64,...

        // Show uploaded image preview in the camera area
        const video = document.getElementById('camera-preview');
        const placeholder = document.getElementById('camera-placeholder');
        let img = document.getElementById('upload-preview');
        if (img) {
            img.src = imageData;
        ...
        const oldCapture = document.getElementById('capture-preview');
        ...
        // Send to server (same endpoint as camera capture)
...
async function captureAndRecognize() {
    ...
    const oldUpload = document.getElementById('upload-preview');
    if (oldUpload) { oldUpload.style.display = 'none'; oldUpload.removeAttribute('src'); }
