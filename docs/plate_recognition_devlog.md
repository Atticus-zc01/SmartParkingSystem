...
### 第 5.3 版 — H 范围加宽（原自适应阈值方案已回退）

**目标**: 尝试提升真实场景检测率（用户反馈~10%成功率）

**尝试方案**: 自适应 HSV 阈值（根据图像亮度动态调整）、H 范围加宽、置信度降至 0.1、margin 扩至 20%

**结果**: ❌ 效果远低于上一版 — 自适应 S/V 在暗光模式下过于宽松（S≥30/V≥25）引入大量假阳性；20% margin 稀释了 LPRNet 输入；0.1 置信度让垃圾结果通过

**回退措施**: 所有参数恢复到上一版已验证的值（S/V、置信度 0.3、margin 15%、面积上限 60000），**仅保留 H 范围加宽这一项无害改进**：
- 蓝色: 95-130（原 100-124）
- 绿色: 30-85（原 35-77）

| 回退项 | 尝试值 | 恢复值 |
|--------|--------|--------|
| 蓝 S/V | 自适应 S≥50/30, V≥40/25 | 固定 S≥60, V≥50 |
| 绿 S/V | 自适应 S≥40/20, V≥30/20 | 固定 S≥50, V≥40 |
| 蓝 H | 95-130 | **95-130（保留）** |
| 绿 H | 30-85 | **30-85（保留）** |
| 最大面积 | 80000 | 60000 |
...
| 文件 | 变更 |
|------|------|
| `src/plate_recognizer.cpp` | `detectPlateCandidates()` H 范围保留加宽，其余全部回退 |
